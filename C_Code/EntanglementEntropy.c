...
void compute_rho_pj(doublecomplex rho_z[4][4], doublecomplex* psit, int ns, int p, int j);
//double compute_concurrence(complex rho_z[4][4]);
//void hermitian_eigenvalues_4x4(complex A[4][4], double evals[4]);
void compute_rho_LR(doublecomplex* rhoL, doublecomplex* psit, int ns, int L);
...
int main()
{
	...
	char* base = "D:\\OneDrive - Tulane University\\RESEARCH\\Quantum transport\\Code\\Plots\\B0LFIMwithLR\\EntanglementTest";
	...
	for (varyVariable = 5; varyVariable < 6; varyVariable += 1)
	{
		...
		for (centerspin = 0; centerspin < 1; centerspin++) //Automating the subtraction process
		{
			...
			double Jlong = 20;// (double)varyVariable / 30;
			...
			for (ns = nsmax; ns <= nsmax; ns += 2) {  //loop over number of spins
				...
				*/
				...
				int perturbedSite = 1; //perturbation on site 2 //this is down

				//psi1
				list1[perturbedSite] = 0; //perturbation on site 2 //not a full spin flip //= list1[7]
				initb1 = cutind(list1);
				//real part of psi
				...
				*/
				...
				for (t = 0; t < 10.01; t += dt) { // start time loop//////////////////////////////////////////////////////////////////////////time
					...
					*
					...
					if (fabs(t * 1000 - round(t * 1000)) < 0.0001)
					{
						...
						if (it % 1000 == 0) {
							int L = ns / 2;          // cut position
							int dL = 1 << L;

							doublecomplex* rhoL = (doublecomplex*)calloc((size_t)dL * dL, sizeof(doublecomplex));
							compute_rho_LR(rhoL, psit, ns, L);

							// write: t ns L  then all entries Re Im
							if (fConcurrence) {
								fprintf(fConcurrence, "%1.15e %d %d", t, ns, L);
								for (int a = 0; a < dL; a++) {
									for (int b = 0; b < dL; b++) {
										doublecomplex z = rhoL[(size_t)a * dL + b];
										fprintf(fConcurrence, " % .15e % .15e", (double)z.r, (double)z.i);
									}
								}
						...
						//			doublecomplex rho[4][4];
						//			/*compute_rho_pj(rho, psit, ns, perturbedSite, k);*/
						//			compute_rho_pj(rho, psit, ns, p, k);
						...
						//				fprintf(fConcurrence, "%1.15e %d %d %0.15f", t, p, k, tr);
						//				for (int a = 0; a < 4; ++a) {
						//					for (int b = 0; b < 4; ++b) {
...
void compute_rho_pj(doublecomplex rho[4][4], doublecomplex* psit, int ns, int p, int j)
{
// ============================================================================
// compute_rho_pj
...
// ============================================================================
	...
	// normalize rho_x in double precision
	double trace_x = rho_x[0][0].r + rho_x[1][1].r + rho_x[2][2].r + rho_x[3][3].r;
	if (trace_x > 1e-14) {
		for (int a = 0; a < 4; ++a)
			for (int b = 0; b < 4; ++b) {
				rho_x[a][b].r /= trace_x;
	...
	// copy normalized double-precision result into the double-precision output
	for (int a = 0; a < 4; ++a) {
		for (int b = 0; b < 4; ++b) {
			rho[a][b].r = rho_x[a][b].r;
			rho[a][b].i = rho_x[a][b].i;
		}
	}
...
void compute_rho_LR(doublecomplex* rhoL, doublecomplex* psit, int ns, int L)
{
	int dim = (int)n;               // number of basis states you evolve over (your truncated basis)
	int dL = 1 << L;               // left Hilbert dimension
	int dR = 1 << (ns - L);        // right Hilbert dimension

	// psi_env[r][a] where:
	//   r = right-basis index (0..dR-1)
	//   a = left-basis index  (0..dL-1)
	doublecomplex** psi_env = (doublecomplex**)malloc((size_t)dR * sizeof(doublecomplex*));
	for (int r = 0; r < dR; r++) {
		psi_env[r] = (doublecomplex*)calloc((size_t)dL, sizeof(doublecomplex));
	}

	int* bits = (int*)malloc((size_t)ns * sizeof(int));

	// Build psi_env
	for (int s = 0; s < dim; s++) {

		// reconstruct spin bitstring for this truncated basis index
		cutspins(s, bits);

		int lidx = 0;
		int ridx = 0;

		// pack left bits [0..L-1] into lidx
		for (int k = 0; k < L; k++) {
			lidx |= (bits[k] << k);
		}
		// pack right bits [L..ns-1] into ridx (shifted down)
		for (int k = L; k < ns; k++) {
			ridx |= (bits[k] << (k - L));
		}

		// accumulate
		psi_env[ridx][lidx].r += psit[s].r;
		psi_env[ridx][lidx].i += psit[s].i;
	}

	free(bits);

	// rhoL[a,b] = sum_r psi_env[r][a] * conj(psi_env[r][b])
	// initialize
	...
	for (int r = 0; r < dR; r++) {
		for (int a = 0; a < dL; a++) {
			double ar = psi_env[r][a].r;
			double ai = psi_env[r][a].i;

			for (int b = 0; b < dL; b++) {
				double br = psi_env[r][b].r;
				double bi = psi_env[r][b].i;

				// (ar + i ai) * conj(br + i bi)
				rhoL[(size_t)a * dL + b].r += (ar * br + ai * bi);
				rhoL[(size_t)a * dL + b].i += (ai * br - ar * bi);
			}
		}
	}

	// normalize by trace (optional but nice)
	double tr = 0.0;
	for (int a = 0; a < dL; a++) tr += rhoL[(size_t)a * dL + a].r;
	...
	// cleanup
	for (int r = 0; r < dR; r++) free(psi_env[r]);
	free(psi_env);
}
