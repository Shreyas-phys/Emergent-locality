...
int main()
{
	...
	char* base = "D:\\OneDrive - Tulane University\\RESEARCH\\Quantum transport\\Code\\Plots\\B0LFIMwithLR\\2dN10Jx5";
	//char* base = "b2N20Jx5Varya0";//// code to send to cypress
	...
	for (varyVariable = 5; varyVariable < 6; varyVariable += 1)
	{
		...
		for (centerspin = 0; centerspin < 1; centerspin++) //Automating the subtraction process
		{
			...
			double Jlong = 5;// (double)varyVariable / 30;
			//double Jz = (double)varyVariable / 10; //NN interaction in z
			double Jz = 1;// 1.0;
			double alpha = 20.0; //short range interaction in x
			double alphalong = (double)varyVariable / 10; //long range interaction in x
			...
			for (ns = nsmax; ns <= nsmax; ns += 2) {  //loop over number of spins
				...
				n = ncutstates;
				int nbonds = 2 * L * (L - 1);  // square lattice OBC bonds: horizontal + vertical
				int nentries = ncutstates + ncutstates * nbonds;               // diagonal entries (one per basis state)
				// worst-case off-diagonal (one per bond)
				nham = nentries;

				//int nentries = ncutstates + ncutstates * ns - choose(ns, maxflips) * (ns - maxflips);// this is for 1D
				nham = nentries;
				...
				*/
				...
				ts = ARR_D(10000000); ds = ARR_D(10000000); d2s = ARR_D(10000000);//was   20000 before
				ds0 = ARR_D(10000000); ds2 = ARR_D(10000000);
				nham = 0;
				...
				int perturbedSite = L+1; //perturbation on site 2

				//psi1
				//(L-1)*(L-1)+L-2
				list1[perturbedSite] = list1[L*L-L-2] = 0; //perturbation on site 2 //not a full spin flip
				initb1 = cutind(list1);
				//real part of psi
				...
				for (i = 0; i < ncutstates; i++) {//if ((mxsave[i] - (ns - 2)) % 4 == 0) {
					...
					for (k = 0; k < ns; k++) {
						...
						// bond to the right (x+1, y)
						if (x + 1 < L) {
							int kp2 = k + 1;

							list2[k] = 1 - list2[k];
							list2[kp2] = 1 - list2[kp2];
					...
					*/
					...
					for (k = 0; k < ns; k++) for (kp = 0; kp < k; kp++) {
						...
						if (fabs(r - 1.0) < 1e-12) {   // nearest neighbors only //////floating point nuissance
							if (list1[k] == list1[kp]) sign = 1; else sign = -1;
							sumV += sign * J;          // r=1 so /pow(r,alpha) does nothing
						}
					}
					...
					for (k = 0; k < ns; k++) for (kp = 0; kp < k; kp++) {
						...
						if (list1[k] == list1[kp]) sign = 1; else sign = -1;
						sumVlong += sign * Jlong / pow(r, alphalong);
					}
					...
					ham2[nham] = sumV + sumVlong - E0; hind1[nham] = i; hind2[nham] = i; nham++;
				}
				...
				*/
				...
				for (t = 0; t < 3.01; t += dt) { // start time loop//////////////////////////////////////////////////////////////////////////time
					...
					*
					...
					// now let's calculate spin dens0ities at time t
					if (fabs(t * 1000 - round(t * 1000)) < 0.00005) {
						//if (1) {
						for (k = 0; k < ns; k++)
						{
